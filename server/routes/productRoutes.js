...
router.post('/', verifyToken, authorizeRoles('Admin', 'Product Manager'), async (req, res) => {
    try {
...
router.put('/:id', verifyToken, authorizeRoles('Admin', 'Product Manager'), async (req, res) => {
    try {
        const product = await Product.findOneAndUpdate({ product_id: req.params.id }, req.body, { new: true });
    ...
    try {
        ...
        for (const productData of dataset.products) {
            try {
                ...
                const existingProduct = await Product.findOne({ product_id: `P${String(productData.id).padStart(3, '0')}` });
